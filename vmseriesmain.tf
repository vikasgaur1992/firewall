...
resource "aws_network_interface" "this" {
  ...
  #security_groups = coalesce(each.value.security_group_ids, [])
  #security_groups = lookup(each.value, "security_group_ids", null)
  #dynamic "security_groups" {
  #  for_each = lookup(each.value, "security_group_ids", null) != null ? [1] : []
  #  content {
  ...
  # Conditional set with dynamic override
  security_groups = try(length(each.value.security_group_ids) > 0, false) ? each.value.security_group_ids : null
}
